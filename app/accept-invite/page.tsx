  ...
  const handleLocalRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.errors || !data.data?.registerWithInvitation?.success) {
        setError(data.data?.registerWithInvitation?.message || 'Failed to create account');
        setIsLoading(false);
      } else {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
        <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 shadow-2xl border border-white/20 max-w-md w-full mx-4 text-center">
          ...
          <p className="text-slate-300 mb-6">This invitation link is invalid or has expired.</p>
          <Link href="/login" className="text-green-400 hover:text-green-300">Go to login</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 shadow-2xl border border-white/20 max-w-md w-full mx-4">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-300">Create your account to join the family tree</p>
        </div>
        ...
        {!useGoogle ? (
          <form onSubmit={handleLocalRegister} className="space-y-4">
            ...
            <div>
              <Label htmlFor="password" className="text-slate-300 mb-1">Password</Label>
              <Input type="password" id="password" value={password} onChange={(e) => setPassword(e.target.value)}
                className="bg-white/10 border-white/20 text-white placeholder-slate-400 focus:ring-green-500"
                placeholder="••••••••" required minLength={8} />
            </div>
            <div>
              <Label htmlFor="confirmPassword" className="text-slate-300 mb-1">Confirm Password</Label>
            ...
            <Button type="button" onClick={() => setUseGoogle(true)}
          ...
          <div className="space-y-4">
            ...
            <Button type="button" variant="ghost" onClick={() => setUseGoogle(false)}
